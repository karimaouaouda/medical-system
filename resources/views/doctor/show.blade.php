            ...
            <div class="pt-20 pb-8 px-8">
                <div class="flex flex-col md:flex-row md:items-start md:justify-between">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div class="bg-gray-50 rounded-lg p-4 text-center">
                                <div class="text-2xl font-bold text-gray-900">{{ $doctor->patients_count }}</div>
                                <div class="text-sm text-gray-500">Patients</div>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-4 text-center">
                                <div class="text-2xl font-bold text-gray-900">{{ $doctor->experience_years }}</div>
                                <div class="text-sm text-gray-500">Years Experience</div>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-4 text-center">
                                <div class="text-2xl font-bold text-gray-900">{{ $doctor->rating }}</div>
                                <div class="text-sm text-gray-500">Rating</div>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-4 text-center">
                                <div class="text-2xl font-bold text-gray-900">{{ $doctor->reviews_count }}</div>
                    ...
                    <div class="mt-8 md:mt-0 md:ml-8 space-y-4">
                        ...
                        <div class="space-y-3">
                            @auth
                                @if(auth()->user()->isFollowing($doctor))
                                    <form action="{{ route('doctor.unfollow', $doctor) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                ...
                                @else
                                    <form action="{{ route('doctor.follow', $doctor) }}" method="POST">
                                        @csrf
                                ...
                                <a href="{{ route('messages.create', ['recipient' => $doctor->id]) }}" 
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="text-gray-900">{{ $doctor->average_response_time }}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Success Rate</span>
                                    <span class="text-gray-900">{{ $doctor->success_rate }}%</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Languages</span>
                                    <span class="text-gray-900">{{ implode(', ', $doctor->languages) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
